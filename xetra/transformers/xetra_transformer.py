"""
File to transform the Xetra data and load the
...
class XetraTargetConfig(NamedTuple):
    ...
    trg_col_isin: column name for time in target
...
class XetraETL():
    """
    Reads the Xetra data, transforms and writes the transformed to the target
    """

    def __init__(self, s3_bucket_src: S3BucketConnector,
                       s3_bucket_trg: S3BucketConnector, meta_key: str,
                       src_args: XetraSourceConfig, trg_args: XetraTargetConfig) -> None:
        """
        Constructor for XetraTransformer

        :param s3_bucket_src: connectin to source S3 bucket
        :param s3_bucket_trg: connectin to target S3 bucket
    ...
    def extract(self):
        ...
        files = [key for date in self.extract_date_list for key in \
            self.s3_bucket_src.list_files_in_prefix(date)]
        if not files:
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        returns:
            data_frame: Transformed data_frame as Output
        """
        if data_frame.empty:
        ...
        # Change of current day's closing price compared to the previous
        # trading day's closing price in %
        ...
        self._logger.info('Applying transformations to Xetra source data is finished...')
        return data_frame
