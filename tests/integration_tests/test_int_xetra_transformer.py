...
class IntTestXetraETLMethods(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.s3_bucket_name_src = 'xetra-landon-int-test-src'
        self.s3_bucket_name_trg = 'xetra-landon-int-test-trg'
        self.meta_key = 'meta_file.csv'
        # Creating source and target bucket instances
        ...
        data = [['AT0000A0E9W5', 'SANT', self.dates[5], '12:00', 20.19, 18.45, 18.20, 20.33, 877],
                ...
                ['AT0000A0E9W5', 'SANT', self.dates[3], '14:00', 18.27, 18.89, 18.27, 21.34, 455],
        ...
        f'{self.dates[5]}/{self.dates[5]}_BINS_XETRA12.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[1:1],
        f'{self.dates[4]}/{self.dates[4]}_BINS_XETRA15.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[2:2],
        f'{self.dates[3]}/{self.dates[3]}_BINS_XETRA13.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[3:3],
        f'{self.dates[3]}/{self.dates[3]}_BINS_XETRA14.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[4:4],
        f'{self.dates[2]}/{self.dates[2]}_BINS_XETRA07.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[5:5],
        f'{self.dates[2]}/{self.dates[2]}_BINS_XETRA08.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[6:6],
        f'{self.dates[1]}/{self.dates[1]}_BINS_XETRA07.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[7:7],
        f'{self.dates[1]}/{self.dates[1]}_BINS_XETRA08.csv','csv')
        self.s3_bucket_src.write_df_to_s3(self.df_src.loc[8:8],
        f'{self.dates[1]}/{self.dates[1]}_BINS_XETRA09.csv','csv')
        ...
        data_report = [['AT0000A0E9W5', self.dates[3], 20.21, 18.27, 18.21,\
                         21.34, 1088, 10.62],
                       ['AT0000A0E9W5', self.dates[2], 20.58, 19.27, 18.89,\
                         21.14, 10286, 1.83],
                       ['AT0000A0E9W5', self.dates[1], 23.58, 24.22, 22.21,\
                        25.01, 3586, 14.58]]
